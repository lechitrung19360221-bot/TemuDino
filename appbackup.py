...
from PySide6.QtGui import QPixmap, QPainter, QWheelEvent, QMouseEvent, QKeyEvent, QFont, QIcon, QGuiApplication
...
class MockupApp(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.global_lock_chk = QCheckBox("Khóa chỉnh sửa (tổng)")
        ...
        self.global_lock_chk.toggled.connect(self.toggle_global_lock)
        lock_row.addWidget(self.lock_design_chk)
        lock_row.addWidget(self.lock_watermark_chk)
        lock_row.addWidget(self.global_lock_chk)
    ...
    def _load_settings(self) -> None:
        ...
        self.global_lock_chk.setChecked(s.value("locks/global", False, type=bool))
        # Upload
    ...
    def _save_settings(self) -> None:
        ...
        s.setValue("locks/global", bool(self.global_lock_chk.isChecked()))
    ...
    def toggle_global_lock(self, locked: bool):
        """Toggle global lock for all interactions"""
        self.global_lock = locked
        self.update_global_lock()
        
    def update_global_lock(self):
        """Update UI state based on global lock"""
        if self.global_lock:
            # Disable all interactive elements
            self.view.setDragMode(QGraphicsView.NoDrag)
            self.pick_mockup_btn.setEnabled(False)
            self.pick_design_btn.setEnabled(False)
            self.pick_watermark_btn.setEnabled(False)
            ...
            # Disable combo boxes
            for combo in [self.anchor_combo, self.aspect_combo, self.blend_combo]:
                combo.setEnabled(False)
                
        else:
            # Re-enable all interactive elements
            self.view.setDragMode(QGraphicsView.ScrollHandDrag)
            self.pick_mockup_btn.setEnabled(True)
            self.pick_design_btn.setEnabled(True)
            self.pick_watermark_btn.setEnabled(True)
            ...
            # Re-apply individual locks
            self.toggle_design_lock(self.design_locked)
            self.toggle_watermark_lock(self.watermark_locked)
            # And refresh transform enabled state
            self._update_transform_controls_enabled()
    ...
    def update_active_item_from_spins(self):
        ...
        if not self.active_item or self.global_lock:
            return
    ...
    def reset_all(self):
        ...
        # Reset locks
        self.global_lock = False
        if hasattr(self, 'global_lock_chk'):
            self.global_lock_chk.setChecked(False)
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.view.viewport():
            if event.type() == QEvent.Wheel:
                ...
                if self.global_lock or not self.active_item:
                    return False
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key_Space:
            # Spacebar to toggle global lock
            self.global_lock_chk.toggle()
